...
public class TurretController : MonoBehaviour
{
    ...
    void AttackTarget(Enemy target)
    {
        ...
        // TUPLA: agrega el sa�o y el tiempo
            ...
            //Lo mantiene siempre loopeando 
        {
                ...
                //Pregunta si hice da�o
            {
...

                {
                    ...
                    // TUPLAS NACHO: da�o y Tiempo 
                    float batchDamage = batch.Sum(tuple => tuple.damage);
                    //calcula cuanto da�o se hizo en el grupo de 10
                    ...
                    //le suma al da�o total, el da�o del batch actual
                    batches++;

                    // Muestro lo que calcule con las tuplas en consola
                    if (timeSpan > 0)
                        //si en algun lapso hice da�o, calcula el dps
                    {
    ...
    // TIPO AN�NIMO NACHO: Genera reporte de acci�n de disparo
    public void GeneratePerformanceReport()
    {
        if (stats.damageTimeline.Count == 0) return;

        // TIPO AN�NIMO: Informaci�n sobre la acci�n de disparo
        var shootingReport = stats.damageTimeline
            .TakeLast(20) // �ltimos 20 disparos
            .Select(tuple => new
            {
                Damage = tuple.damage,
                Time = tuple.time,
        ...
        if (shootingReport.Any())
            //levanta el da�o para dar la performance,dando un resumen general de las estadisticas de los ataques.
        {
            var summary = new
            {
    ...
    // AGGREGATE NACHO: Calcula estad�sticas complejas usando Aggregate
    public void CalculateAdvancedStats()
    {
        if (stats.damageHistory.Count == 0) return;

        //AGGREGATE: Combina m�ltiples c�lculos en una sola operaci�n
        var complexStats = stats.damageHistory
            .TakeLast(15) // �ltimos 15 disparos
            .Aggregate(
                ...
                (accumulator, damage) => new
                {
                    ...
                    DamageVariation = accumulator.DamageVariation + Mathf.Abs(damage - 20f) // Variaci�n del da�o base
                },
                result => new // Transformaci�n final
                {
    ...
    // GENERATOR NACHO: Genera proyecciones de da�o futuro de forma lazy
    IEnumerator DamageProjectionGenerator()
    {
        while (true)
        {
            yield return new WaitForSeconds(7f); // Cada 7 segundos genera proyecci�n

            if (stats.damageHistory.Count >= 5) // Necesita al menos 5 disparos para proyectar
            {
                // Generator que calcula da�o proyectado para los pr�ximos 5 disparos
    ...
    IEnumerable<float> GenerateFutureDamageProjections()
    {
        ...
        {
            // Proyecta daño basado en tendencia + variacion por shot
            float projectedDamage = trend + (shotNumber * 0.5f) - 2.5f; // Pequeña variacion
            projectedDamage = Mathf.Max(projectedDamage, baseDamage * 0.8f); // Monimo 80% del da�o base
